<?php global $tmc_option; 
		...
		{
			$backStyle = '';
			if(isset($tmc_option['footer4_widget']) && $tmc_option['footer4_widget'] == '1') 
			{ 
				$backStyle = backgroundStyle('footer4_bg');
	...
	if($metaData['hide-footer'] != 'yes')
	{
		...
		{	$footerBg = ''; 
			...
			if($tmc_option['footer4_widget'])
			{
			?>
				<footer class="footer4" style="<?php echo esc_attr($backStyle); ?>">
					<div class="ftr-section" style="<?php echo esc_attr($backStyle); ?>">
						<div class="container">
					<?php 	if($tmc_option['footer4_info_switch'])
							{ ?>
								<ul class="footer-info">
							<?php	if(isset($tmc_option['footer4_add_line_one']) && $tmc_option['footer4_add_line_one'] != '' || isset($tmc_option['footer4_add_line_two']) && $tmc_option['footer4_add_line_two'] != '' ):?>
										<li class="<?php echo esc_attr($tmc_option['footer4_add_icon']); ?>">
											<?php echo esc_attr($tmc_option['footer4_add_line_one']); ?> <br/> <?php echo esc_attr($tmc_option['footer4_add_line_two']); ?>
										</li>
							<?php 	endif; 
									if(isset($tmc_option['footer4_phn']) && $tmc_option['footer4_phn'] != ''): ?>
										<li class="<?php echo esc_attr($tmc_option['footer4_phn_icon']); ?>">
											<?php echo esc_attr($tmc_option['footer4_phn']); ?>
										</li>
							<?php 	endif; 
									if(isset($tmc_option['footer4_email']) && $tmc_option['footer4_email'] != ''):?>
										<li class="<?php echo esc_attr($tmc_option['footer4_email_icon']); ?>">
											<?php echo esc_attr($tmc_option['footer4_email']); ?>
										</li>
							<?php 	endif; 
									if(isset($tmc_option['footer4_time']) && $tmc_option['footer4_time'] != ''):?>
										<li class="<?php echo esc_attr($tmc_option['footer4_time_icon']); ?>">
											<?php echo esc_attr($tmc_option['footer4_time']); ?>
										</li>
							<?php 	endif; ?>		
								</ul>
					<?php	} 
							$footer4_sidebar_count = intval( $tmc_option['footer4_sidebar_count'] );
							$col = 12 / $footer4_sidebar_count;	?>
							<div class="row">
							<?php	for ( $count = 1; $count <= $footer4_sidebar_count; $count ++ ):
										if($count == 1)
										{
											$col_class = '4';
					...
					{
						if( isset($tmc_option['footer4_copyright_switch']) && $tmc_option['footer4_copyright_switch'] == '1' ): ?>
									<div class="footer-btm">
								<?php 	if( isset($tmc_option['footer4_copyright']) && $tmc_option['footer4_copyright'] != '' )
										{ ?>
											<div class="col-md-6 col-sm-6 col-xs-12 pad-left_zero pad-right_zero">
												<p><?php echo esc_attr($tmc_option['footer4_copyright']); ?></p>
											</div>
								<?php 	}
										if( isset($tmc_option['footer4_develop_text']) && $tmc_option['footer4_develop_text'] != '' )
										{ ?>		
											<div class="col-md-4 col-sm-6 col-xs-12 pad-left_zero pad-right_zero pull-right">
												<p class="text-right">
													<a href="<?php echo esc_url($tmc_option['footer4_develop_link']); ?>">
														<?php echo esc_attr($tmc_option['footer4_develop_text']); ?>
													</a>
												</p>
											</div>
								<?php 	} ?>
