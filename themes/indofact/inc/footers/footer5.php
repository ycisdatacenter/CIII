<?php global $tmc_option; 
		...
		{
			$backStyle = '';
			if(isset($tmc_option['footer5_widget']) && $tmc_option['footer5_widget'] == '1') 
			{ 
				$backStyle = backgroundStyle('footer5_bg');
	...
	if($metaData['hide-footer'] != 'yes')
	{
		...
		{	$footerBg = ''; 
			...
			if($tmc_option['footer5_widget'])
			{
			?>
				<footer class="footer5" style="<?php echo esc_attr($backStyle); ?>">

					<div class="ftr-section" style="<?php echo esc_attr($backStyle); ?>">
						<div class="container">
							<?php 	
					      if($tmc_option['footer5_strip'])
							{
								?>
								<div class="plumber_footer_strip">
									<h5>
										<?php echo wp_kses_post($tmc_option['footer5_strip_content']); ?>
									</h5>
									<?php 
									if(isset($tmc_option['footer5_form_id']) && get_the_title($tmc_option['footer5_form_id']) != '') 
									{ ?>
										<div class="plumber_footer_form">
											<?php echo do_shortcode('[contact-form-7 id="'.esc_attr($tmc_option['footer5_form_id']).'" title="Footer Form"]'); ?>
										</div>
							      <?php 	
							      } ?>
								</div>
					      <?php 	
							...
							$footer5_sidebar_count = intval( $tmc_option['footer5_sidebar_count'] );
							$col = 12 / $footer5_sidebar_count;	?>
							<div class="footer_middle">
								<div class="row">
								<?php	for ( $count = 1; $count <= $footer5_sidebar_count; $count ++ ):
											if($count == 1)
											{
												$col_class = '4';
					...
					{
						if( isset($tmc_option['footer5_copyright_switch']) && $tmc_option['footer5_copyright_switch'] == '1' ): ?>
									<div class="footer-btm">
								<?php 	if( isset($tmc_option['footer5_copyright']) && $tmc_option['footer5_copyright'] != '' )
										{ ?>
											<div class="col-md-6 col-sm-6 col-xs-12 pad-left_zero pad-right_zero">
												<p><?php echo esc_attr($tmc_option['footer5_copyright']); ?></p>
											</div>
								<?php 	}
										if( isset($tmc_option['footer5_develop_text']) && $tmc_option['footer5_develop_text'] != '' )
										{ ?>		
											<div class="col-md-4 col-sm-6 col-xs-12 pad-left_zero pad-right_zero pull-right">
												<p class="text-right">
													<a href="<?php echo esc_url($tmc_option['footer5_develop_link']); ?>">
														<?php echo esc_attr($tmc_option['footer5_develop_text']); ?>
													</a>
												</p>
											</div>
								<?php 	} ?>
