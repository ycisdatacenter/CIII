...
class WooCommercePluginsRepository
{
    ...
    {
        return WooCommerceRepository::isWooCommerceActive() ? \WC_Helper::get_local_woo_plugins() : [];
    }
    ...
    {
        return WooCommerceRepository::isWooCommerceActive() ? \WC_Helper::get_subscriptions() : [];
    }

    /**
     * Gets the connected site ID.
     */
    public static function getWooCommerceConnectedSiteId() : ?int
    {
        $auth = ArrayHelper::wrap(\WC_Helper_Options::get('auth'));

        return ArrayHelper::getIntValueForKey($auth, 'site_id') ?: null;
    }
