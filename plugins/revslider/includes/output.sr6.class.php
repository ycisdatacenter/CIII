...
class RevSliderOutput extends RevSliderFunctions {
	...
	public function get_spinner_markup(){
		...
		switch($spinner){
			case '1':
			case '2':
				$css_html = "#".$this->get_html_id()."_wrapper rs-loader.spinner".$spinner."{ background-color: ". esc_attr($color) ." !important; }";
				$html = $this->get_css_javascript($css_html);
			break;
			case '3':
			case '4':
				$css_html = "#".$this->get_html_id()."_wrapper rs-loader.spinner".$spinner." div { background-color: ". esc_attr($color) ." !important; }";
				$html = $this->get_css_javascript($css_html);
			break;
			case '0':
			case '5':
	...
	public function write_js_var($v, $pp = '"'){
		...
		return (is_numeric($v) || substr($v, 0, 1) === '[' || in_array($v, array('true', 'false'))) ? $v : $pp . esc_js($v) . $pp;
	}
}
